...
class Battle
  ...
  def pbTerastallize(idxBattler)
    ...
    pbDisplay(_INTL("¡{1} se Teracristalizo en {2}-type!", battler.pbThis, GameData::Type.get(battler.tera_type).name))
  ...
  def pbAnimateTerastallization(battler)
    if @scene.pbCommonAnimationExists?("Terastallize")
      ...
      if Settings::SHOW_TERA_ANIM && $PokemonSystem.battlescene == 0
        ...
        if battler.wild?
          pbDisplay(_INTL("¡{1} se rodeo de energia Terastal!", battler.pbThis))
        else
          trainerName = pbGetOwnerName(battler.index)
          pbDisplay(_INTL("¡{1} esta reaccionado al {3} de {2}!", battler.pbThis, trainerName, pbGetTeraOrbName(battler.index)))
        end
