#===============================================================================
# Agrega un nuevo efecto para almacenar los datos del Pokémon en el que se transformó el usuario.
#===============================================================================
module PBEffects
...
class Battle::Move
  ...
  #-----------------------------------------------------------------------------
  # Inicializa los nombres de movimiento acortados para movimientos con nombres muy largos.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility utilizada para verificar los movimientos de movimientos z/Dynamax, si existe.
  #-----------------------------------------------------------------------------
  def powerMove?
    return true if defined?(zMove?) && zMove?
    return true if defined?(dynamaxMove?) && dynamaxMove?
...
class Battle
  #-----------------------------------------------------------------------------
  # Utility para verificar si algún luchador en un lado en particular está a bajo HP.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para verificar si un entrenador le queda algún Pokémon disponible en el equipo.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para devolver una matriz de cada luchador propiedad de un entrenador en particular.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Edita mensajes de elementos para un uso más descriptivo.
  #-----------------------------------------------------------------------------
...
class Battle::Battler
  ...
  #-----------------------------------------------------------------------------
  # Inicializar las propiedades utilizadas por varias características del complemento.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para conseguir el Pokémon que una luchadora está mostrando como.
  #-----------------------------------------------------------------------------
  def visiblePokemon
  ...
  #-----------------------------------------------------------------------------
  # Utility para verificar si el battler está en HP bajo.
  #-----------------------------------------------------------------------------
  def hasLowHP?
  ...
  #-----------------------------------------------------------------------------
  # Alias ​​para actualizar BGM cuando el HP de las actualizaciones de Battler del jugador.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para reiniciar un battler de regreso a su conjunto de movimientos original.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utilities para verificar la compatibilidad con acciones especiales.
  #-----------------------------------------------------------------------------
  def getActiveState
    return :mega      if mega?
  ...
  #-----------------------------------------------------------------------------
  # Utility para refrescar la forma de un luchador.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Se utiliza para verificar si el battler puede proteger contra un movimiento específico.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para evolucionar un Pokemon a mitad de una batalla..
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Similar a PbChangeForm, excepto que ignora el aprendizaje de nuevos movimientos para ciertas especies.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Verifica los cambios de formulario al cambiar el elemento de Battler..
  #-----------------------------------------------------------------------------
  def pbCheckFormOnHeldItemChange
    ...
    if isSpecies?(:DIALGA)
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} se transformo!", pbThis))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:ARCEUS) && self.ability == :MULTITYPE
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} adquirio el tipo {2}!", pbThis, type.name))
    end
    #---------------------------------------------------------------------------
    ...
    if isSpecies?(:OGERPON)
      ...
      pbSimpleFormChange(newForm, _INTL("¡{1} se puso la {2}!", pbThis, maskName))
    end
  end
end

#===============================================================================
# Trainer Battle Llama que hace que el jugador seleccione varios Pokémon para la batalla.
#===============================================================================
class TrainerBattle
  def self.select_start(size, *args)
    ...
    if $player.able_pokemon_count < size
      pbMessage(_INTL("#{g}No tienes suficientes Pokémons en tu equipo que puedan participar ..."))
      pbMessage(_INTL("#{g}Vuelve cuando tengas suficientes Pokémons para luchar."))
      return nil
    else
      new_party = nil
      ...
      if new_party
        ...
        pbMessage(_INTL("#{g}¿Eh?¿Cambiaste de opinión? "))
        pbMessage(_INTL("#{g}Vuelve cuando tengas el Pokéméon correcto con el que quieras luchar."))
        return nil
      end
    end
  end
...
class Battle::AI::AITrainer
  #-----------------------------------------------------------------------------
  # Alias ​​para darle a Pokemon Wild AI mejor cuando se habilita un modo de batalla salvaje.
  #-----------------------------------------------------------------------------
...
class Battle::AI::AIBattler
  #-----------------------------------------------------------------------------
  # Utilities para realizar controles en los luchadores opositores.
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  # Utility para verificar si un luchador está en riesgo de obtener un efecto de estado.
  #-----------------------------------------------------------------------------
...
#===============================================================================
# Utilities para manejar las evoluciones de la mitad de la batalla.
#===============================================================================
class PokemonEvolutionScene
  def pbBattleEvolution(battler)
    ...
    pbPlayDecisionSE
    @pokemon.play_cry
    @sprites["msgwindow"].text = _INTL("¡{1} esta evolucionando!", battler.pbThis)
  ...
  def pbBattleEvolutionSuccess(battler)
    ...
    if battler.pbOwnedByPlayer?
      ...
      if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
        ...
        pbMessageDisplay(@sprites["msgwindow"],
                         _INTL("Los datos de {1}se añadieron a la Pokédex.", newspeciesname)) { pbUpdate }
