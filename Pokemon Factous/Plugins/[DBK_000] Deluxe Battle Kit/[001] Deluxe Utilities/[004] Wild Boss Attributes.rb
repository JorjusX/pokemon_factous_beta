...
class Battle::Battler
  ...
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    if (immunities.include?(:ALLSTATUS) || immunities.include?(newStatus)) && !self_inflicted && !ignoreStatus
      case newStatus
      when :SLEEP     then msg = _INTL("¡{1} es completamente inmune a ser puesto a dormir!", pbThis)
      when :POISON    then msg = _INTL("¡{1} es completamente inmune al envenenamiento!", pbThis)
      when :BURN      then msg = _INTL("¡{1} es completamente inmune a las quemaduras!", pbThis)
      when :PARALYSIS then msg = _INTL("¡{1} es completamente inmune a la parálisis!", pbThis)
      when :FROZEN    then msg = _INTL("¡{1} es completamente inmune a estar congelado!", pbThis)
      when :FROSTBITE then msg = _INTL("¡{1} es completamente inmune a la congelación!", pbThis)
      when :DROWSY    then msg = _INTL("¡{1} es completamente inmune a ser somnoliento!", pbThis)
      end
  ...
  def pbCanLowerStatStage?(*args)
    ...
    if (!args[1] || args[1].index != @index) && 
      ...
      @battle.pbDisplay(_INTL("¡{1} es completamente inmune a la bajada de estadisticas!", pbThis)) if args[3]
      return false
    end
  ...
  def takesIndirectDamage?(showMsg = false)
    ...
    if @pokemon.immunities.include?(:INDIRECT)
      @battle.pbDisplay(_INTL("¡{1} es completamente inmune al daño indirecto!", pbThis)) if showMsg
      return false
    end
