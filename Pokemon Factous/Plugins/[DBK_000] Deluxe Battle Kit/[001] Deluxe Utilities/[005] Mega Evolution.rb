#===============================================================================
# Todos los cambios en el código relacionados con Mega Evolution.
# Esto se usa para estandarizar la mecánica de la evolución de Mega con las mismas características
# incluido con otras mecánicas de batalla agregadas por complementos compatibles.
#===============================================================================

#-------------------------------------------------------------------------------
# Seguimiento de estadísticas del juego para mega batallas Wild Mega.
#-------------------------------------------------------------------------------
class GameStats
...
#-------------------------------------------------------------------------------
# Muestra un icono de elemento retenido para mega piedras en el Party Menu.
#-------------------------------------------------------------------------------
module GameData
  class Item
...
#-------------------------------------------------------------------------------
# Actualizaciones a Mega Evolution Battle Scripts.
#-------------------------------------------------------------------------------
class Battle
  ...
  def pbMegaEvolve(idxBattler)
    ...
    if battler.wild?
      ...
      when 1
        pbDisplay(_INTL("¡{1} irradia con mega energía!", battler.pbThis))
      else
        pbDisplay(_INTL("¡La {2} de {1} se irradia con Mega Energia!", battler.pbThis, battler.itemName))
      end
    else
      trainerName = pbGetOwnerName(idxBattler)
      case battler.pokemon.megaMessage
      when 1
        pbDisplay(_INTL("¡El ferviente de {1} deseo ha alcanzado {2}!", trainerName, battler.pbThis))
      else
...
#-------------------------------------------------------------------------------
# Mega Evolution Battler Elegibilidad Check.
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# Muestra un nuevo icono de Mega Evolution en Databoxes de Battler.
#-------------------------------------------------------------------------------
...
#===============================================================================
# Animación de batalla por desencadenar la mega evolución.
#===============================================================================
class Battle::Scene::Animation::BattlerMegaEvolve < Battle::Scene::Animation
  #-----------------------------------------------------------------------------
  # Inicializa los datos utilizados para la animación.
  #-----------------------------------------------------------------------------
  def initialize(sprites, viewport, idxBattler, battle)
    #---------------------------------------------------------------------------
    # Obtiene datos de Pokémon del índice Battler.
    @battle = battle
    ...
    #---------------------------------------------------------------------------
    # Obtiene datos del entrenador del índice de battler (no solo wild).
    if !@battler.wild?
    ...
    #---------------------------------------------------------------------------
    # Obtiene datos de antecedentes y animación.
  ...
  def createProcesses
    ...
    #---------------------------------------------------------------------------
    # Establece el entrenador y el mega anillo                                        
    if !@battler.wild?
    ...
    #---------------------------------------------------------------------------
    # Configuración del botón Skip & Fade Out.
    picBUTTON = dxSetSkipButton(delay)
    picFADE = dxSetFade(delay)
    ############################################################################
    # Inicio de animación.
    ############################################################################
    # Se desvanece en la escena.
    ...
    #---------------------------------------------------------------------------
    # Diapositivas de entrenador en la pantalla con base (no solo Wild).
    if !@battler.wild?
      ...
      #-------------------------------------------------------------------------
      # Mega Ring aparece con contorno;Deslice hacia arriba.
    ...
    #---------------------------------------------------------------------------
    # Mega Stone aparece con contorno;Deslice hacia arriba.
    ...
    #---------------------------------------------------------------------------
    # Oscurece el fondo/tono base;ilumina Pokémon a blanco.
    ...
    #---------------------------------------------------------------------------
    # Las partículas comienzan a dibujar a Pokémon.
    repeat = delay
    2.times do |t|
      repeat -= 4 if t > 0
    ...
    #---------------------------------------------------------------------------
    # White Orb Engulfs Pokemon;aparecen grietas;Orb se expande lejos de Pokémon.
    ...
    #---------------------------------------------------------------------------
    # Flash de pantalla blanca;muestra la silueta de Mega Pokemon.
    ...
    delay = picFADE.totalDuration
    #---------------------------------------------------------------------------
    # Mega Pokemon reveló;El pulso se expande hacia afuera;Overlay y mega icono se muestran.
    ...
    #---------------------------------------------------------------------------
    # Shakes Pokemon;juega llorar;Flashes superpuestos.Se desvanece.
    16.times do |i|
      if i > 0
